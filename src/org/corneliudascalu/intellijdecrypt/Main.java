    ...
    public static void main(String[] args) throws Exception {
        ...
        if (passwords.getLength() > 1) {
            ...
            for (int i = 1; i < passwords.getLength(); i++) {
                NamedNodeMap attrs = passwords.item(i).getAttributes();
                Node attrValue = attrs.getNamedItem("value");
                if (attrValue != null) {
                    System.out.println(attrValue.getNodeValue());
                    try {
                        System.out.println(EncryptionUtil.decryptText(EncryptionUtil.genPasswordKey(password),
                                fromHex(attrValue.getNodeValue())));
